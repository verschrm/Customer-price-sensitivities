...
for (m in 1:M) {
  # Augment the dataset
  AMI_Augm$Churn <- as.vector(t(Augm_Response[, , m]))
  # Estimate a global logistic GLM
  GLM_MI <- glm(MI_formula, data = AMI_Augm,
                family = binomial(link = 'logit'), control = list(maxit = 500))
  # Store the parameter estimates from the imputation
  beta_MI[[m]] <- as.matrix(coef(GLM_MI))
  W_MI[[m]] <- as.matrix(diag(vcov(GLM_MI)))
}

### Combine estimates from multiple imputation
beta_MI <- t(matrix(unlist(beta_MI), length(beta_MI[[1]]), M))
W_MI <- t(matrix(unlist(W_MI), length(W_MI[[1]]), M))
beta_hat <- sapply(1:ncol(beta_MI), function(z) sum(beta_MI[, z]) / M)
W_hat <- sapply(1:ncol(W_MI), function(z) sum(W_MI[, z]) / M)
B_hat <- sapply(1:ncol(beta_MI), function(z) sum((beta_MI[, z] - beta_hat[z]) ^ 2) / (M - 1))
sig2_hat <- W_hat + (1 + 1 / M) * B_hat

### Discrete global response model
# Estimate a global response model
Logit_GLM <- glm(MI_formula, data = AMI, family = binomial(link = 'logit'))
# Overwrite parameter estimates with estimates from multiple imputation
names(beta_hat) <- names(Logit_GLM$coefficients)
Logit_GLM$coefficients <- beta_hat
