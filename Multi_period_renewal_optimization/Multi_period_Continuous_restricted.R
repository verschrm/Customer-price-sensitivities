...
# Load results from Frontier_Continuous_restricted.R
...
NLP_obj_con_i_tau <- function(Offers, P_Old, Offer_Grid, Data_i_tau, Ind_t, Churn_Model, 
                              Costs_i, Mean_i_1, Normalizer_i_1, GPS_Model, sd_GPS,
                              eps, alpha) {
  ...
  # Estimated churn rate
  Churn_i_1 <- NLP_Churn_i_1(Offers = Offer_Grid[Cat_Offers[Ind_t[[1]]]], sd_GPS = sd_GPS, 
                             Data_i_1 = Data_i_tau[Ind_t[[1]], ], Mean = Mean_i_1, 
                             Normalizer = Normalizer_i_1, Churn_Model = Churn_Model)
  Churn_i_2 <- NLP_Churn_i_t(P_Old_t = P_Old_2, GPS_Model = GPS_Model, 
                             Offers = Offer_Grid[Cat_Offers[Ind_t[[2]]]], sd_GPS = sd_GPS, 
                             Data_i_t = Data_i_tau[Ind_t[[2]], ], Churn_Model = Churn_Model)
  Churn_i_3 <- NLP_Churn_i_t(P_Old_t = P_Old_3, GPS_Model = GPS_Model, 
                             Offers = Offer_Grid[Cat_Offers[Ind_t[[3]]]], sd_GPS = sd_GPS, 
                             Data_i_t = Data_i_tau[Ind_t[[3]], ], Churn_Model = Churn_Model)
...
Q_means <- sapply(1:length(Q_buckets), function(t) (Q_bins[t] + Q_bins[t + 1]) / 2)
...
Churn_Act_1 <- NLP_Churn_i_1(Offers = AMI_i$Offer[Ind_t_1], sd_GPS = sd_GPS_XGB, Data_i_1 = Data_i_1, 
                             Mean = Mean_1, Normalizer = Normalizer_1, Churn_Model = Churn_XGB)
...
for (t_1 in 1:T_max) {
  ...
  for (t_2 in 1:T_max) {
    ...
    Churn_i_3 <- t(sapply(1:T_max, function(t_3) 
      NLP_Churn_i_t(P_Old_t = P_Old_3, GPS_Model = GPS_XGB, Offers = Offer_Grid[t_3], 
...
# Churn
Churn_LP_1 <- NLP_Churn_i_1(Offers = Offer_Grid[Ind_LP_1], sd_GPS = sd_GPS_XGB, 
                            Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                            Churn_Model = Churn_XGB)
Churn_LP_2 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Results_Offers_1[1, ]), 
                            GPS_Model = GPS_XGB, Offers = Offer_Grid[Ind_LP_2],
                            sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                            Churn_Model = Churn_XGB)
Churn_LP_3 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Results_Offers_1[1, ]) *
                              (1 + Results_Offers_1[2, ]),
                            GPS_Model = GPS_XGB, Offers = Offer_Grid[Ind_LP_3], 
                            sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_3,
                            Churn_Model = Churn_XGB)
# Profit
Profit_LP <- list()
...
NLP_obj_con <- function(x) {
  NLP_obj_con_i_tau(Offers = x, P_Old = P_Old_Base, Offer_Grid = Offer_Grid, 
                    Data_i_tau = AMI_i, Ind_t = Ind_t, Churn_Model = Churn_XGB, 
                    Costs_i = Costs_i, Mean_i_1 = Mean_1, Normalizer_i_1 = Normalizer_1, 
                    GPS_Model = GPS_XGB, sd_GPS = sd_GPS_XGB, eps = NLP_eps, alpha = alpha_t)
}
...
# Churn
Churn_NLP_1 <- NLP_Churn_i_1(Offers = Offer_Grid[NLP_sol[Ind_t_1]], sd_GPS = sd_GPS_XGB, 
                             Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                             Churn_Model = Churn_XGB)
Churn_NLP_2 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]), 
                             GPS_Model = GPS_XGB, Offers = Offer_Grid[NLP_sol[Ind_t_2]],
                             sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2, 
                             Churn_Model = Churn_XGB)
Churn_NLP_3 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]) *
                               (1 + Offer_Grid[NLP_sol[Ind_t_2]]),
                             GPS_Model = GPS_XGB, Offers = Offer_Grid[NLP_sol[Ind_t_3]],
                             sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_3,
                             Churn_Model = Churn_XGB)
# Profit
Profit_NLP <- list()
Profit_NLP[[1]] <- (1 - Churn_NLP_1) * (P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]) - Costs_1)
Profit_NLP[[2]] <- (1 - Churn_NLP_1) * (1 - Churn_NLP_2) * 
  (P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]) * (1 + Offer_Grid[NLP_sol[Ind_t_2]]) - Costs_2)
...
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]) *
                  (1 + Offer_Grid[NLP_sol[Ind_t_2]]),
                GPS_Model = GPS_XGB, Offers = Offer_Grid[t],
                sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_3,
                Churn_Model = Churn_XGB)))
...
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]), 
                GPS_Model = GPS_XGB, Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_2, Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[NLP_sol[Ind_t_1]]) *
                  (1 + Offer_Grid[t]), GPS_Model = GPS_XGB, Offers = LP_extra_3[3, ],
                sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
Churn_i_1 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_1(Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, Data_i_1 = Data_i_1, 
                Mean = Mean_1, Normalizer = Normalizer_1, Churn_Model = Churn_XGB)))
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]), GPS_Model = GPS_XGB, 
                Offers = LP_extra_2[3, ], d_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]) * (1 + LP_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = LP_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
# Period 2
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_extra_1[3, ]), GPS_Model = GPS_XGB, 
                Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_extra_1[3, ]) * (1 + Offer_Grid[t]),
                GPS_Model = GPS_XGB, Offers = LP_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
LP_2_extra_2 <- LP_Rsymphony_optim_i_1(Remain_i_t = (1 - Churn_Loop_1), 
                                       ...
                                       Offers = Offer_Grid)
# Period 3
Churn_i_2 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_extra_1[3, ]), 
                           GPS_Model = GPS_XGB, Offers = LP_2_extra_2[3, ],
                           sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                           Churn_Model = Churn_XGB)
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_extra_1[3, ]) * (1 + LP_2_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_extra_1[3, ]) * (1 + Offer_Grid[t]),
                GPS_Model = GPS_XGB, Offers = LP_2_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
P_Old_2 <- P_Old_Base * (1 + LP_extra_1[3, ])
...
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]), GPS_Model = GPS_XGB, 
                Offers = LP_3_extra_2[3, ], sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]) * (1 + LP_3_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = LP_2_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
LP_2_extra_1 <- LP_Rsymphony_optim_i_1(Remain_i_t = 1, 
                                       ...
                                       Offers = Offer_Grid)
# Period 2
Churn_i_1 <- NLP_Churn_i_1(Offers = LP_2_extra_1[3, ], sd_GPS = sd_GPS_XGB, 
                           Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                           Churn_Model = Churn_XGB)
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_2_extra_1[3, ]), GPS_Model = GPS_XGB, 
                Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_2_extra_1[3, ]) * (1 + Offer_Grid[t]),
                GPS_Model = GPS_XGB, Offers = LP_2_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
Churn_i_1 <- NLP_Churn_i_1(Offers = LP_2_extra_1[3, ], sd_GPS = sd_GPS_XGB, 
                           Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                           Churn_Model = Churn_XGB)
Churn_i_2 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_2_extra_1[3, ]), 
                           GPS_Model = GPS_XGB, Offers = LP_4_extra_2[3, ],
                           sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                           Churn_Model = Churn_XGB)
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_2_extra_1[3, ]) * (1 + LP_4_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_2_extra_1[3, ]) * (1 + Offer_Grid[t]),
                GPS_Model = GPS_XGB, Offers = LP_3_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
P_Old_2 <- P_Old_Base * (1 + LP_2_extra_1[3, ])
...
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]), GPS_Model = GPS_XGB, 
                Offers = LP_5_extra_2[3, ], sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]) * (1 + LP_5_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = LP_3_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
LP_4_extra_1 <- LP_Rsymphony_optim_i_1(Remain_i_t = 1, 
                                       ...
                                       Offers = Offer_Grid)
# Period 2
Churn_i_1 <- NLP_Churn_i_1(Offers = LP_3_extra_1[3, ], sd_GPS = sd_GPS_XGB, 
                           Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                           Churn_Model = Churn_XGB)
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_3_extra_1[3, ]), GPS_Model = GPS_XGB, 
                Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_3_extra_1[3, ]) * (1 + Offer_Grid[t]),
                GPS_Model = GPS_XGB, Offers = LP_3_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
Churn_i_1 <- NLP_Churn_i_1(Offers = LP_3_extra_1[3, ], sd_GPS = sd_GPS_XGB, 
                           Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                           Churn_Model = Churn_XGB)
Churn_i_2 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_3_extra_1[3, ]),
                           GPS_Model = GPS_XGB, Offers = LP_6_extra_2[3, ],
                           sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2, 
                           Churn_Model = Churn_XGB)
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_3_extra_1[3, ]) * (1 + LP_6_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = Offer_Grid[t], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
...
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_3_extra_1[3, ]) * (1 + Offer_Grid[t]),
                GPS_Model = GPS_XGB, Offers = LP_4_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
P_Old_2 <- P_Old_Base * (1 + LP_3_extra_1[3, ])
...
Churn_i_2 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]), GPS_Model = GPS_XGB, 
                Offers = LP_7_extra_2[3, ], sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                Churn_Model = Churn_XGB)))
Churn_i_3 <- t(sapply(1:T_max, function(t)
  NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + Offer_Grid[t]) * (1 + LP_7_extra_2[3, ]),
                GPS_Model = GPS_XGB, Offers = LP_4_extra_3[3, ], sd_GPS = sd_GPS_XGB, 
                Data_i_t = Data_i_3, Churn_Model = Churn_XGB)))
LP_4_extra_1 <- LP_Rsymphony_optim_i_1(Remain_i_t = 1, 
                                       Profit_i_t = t(t(1 - Churn_i_1) * (t(1 - Churn_i_2) * 
...
# Churn
Churn_LP_Add_1 <- NLP_Churn_i_1(Offers = LP_4_extra_1[3, ], sd_GPS = sd_GPS_XGB, 
                                Data_i_1 = Data_i_1, Mean = Mean_1, Normalizer = Normalizer_1, 
                                Churn_Model = Churn_XGB)
Churn_LP_Add_2 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_4_extra_1[3, ]), 
                                GPS_Model = GPS_XGB, Offers = LP_7_extra_2[3, ],
                                sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_2,
                                Churn_Model = Churn_XGB)
Churn_LP_Add_3 <- NLP_Churn_i_t(P_Old_t = P_Old_Base * (1 + LP_4_extra_1[3, ]) * 
                                  (1 + LP_7_extra_2[3, ]),
                                GPS_Model = GPS_XGB, Offers = LP_4_extra_3[3, ],
                                sd_GPS = sd_GPS_XGB, Data_i_t = Data_i_3,
                                Churn_Model = Churn_XGB)
# Profit
Profit_LP_Add <- list()
